...
最多保留三类 baseline family：

1. **Direct ternary PTQ**：当前 direct ternary 起点；
2. **Strong ternary PTQ**：优先 PT² 官方实现/论文设置，或者仓库中可复现的 GPTQ-style ternary baseline；
3. **QAT reference**：只作为上界和 gap 参照，不向 CEGSP 提供 teacher、logits、checkpoint 或 latent weights。

PT² 如果由于架构、版本或输入接口无法公平迁移，必须单独记录 `baseline-unavailable`，不能把 CEGSP 对弱 baseline 的提升写成 SOTA。
...
把当前零散的 OPT scale 和 Pythia cross-architecture 结果合并成论文主表，而不是继续做更多局部 ablation。
...
- strong ternary PTQ baseline（若实现完成）。
...
内容：统一 split 记录、direct parity、Pythia adapter parity、strong ternary PTQ baseline 接口确认、cost timer。预计 0.5–1 GPU 小时，主要是工程工作。
...
内容：OPT-1.3B、OPT-2.7B、Pythia-1B；direct、CEGSP joint/support/signflip；`k25/k50`；Wikitext/C4 holdout。优先复用缓存，预计 2–4 GPU 小时。

**决策：** 通过 B1 才进入论文主结果；部分通过则收窄架构/预算表述，不增加模块。
