#!/usr/bin/env python3
"""E1: cross-model complete TernRefine APG validation.

This script uses the ordinary affine ternary initializer from P7, computes one
quantized-point CE gradient over all Q/K decoder projections, freezes the QGP
ranking, and selects the patch extent with APG on a held-out validation split.
"""
...
def nonconflicting_prefix(edits: Sequence[AffineEdit], limit: int) -> List[AffineEdit]:
    """Keep the highest-ranked legal moves without reusing a ternary group."""
    out: List[AffineEdit] = []
    used = set()
    for edit in edits:
        group_key = (edit.layer, edit.key, edit.row, edit.block)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--mode", choices=["smoke", "apg"], default="apg")
    ...
    p.add_argument("--apg-ks", default="1,2,4,8,16,32,64")
    p.add_argument("--top-k-per-module", type=int, default=256)
...
def main() -> None:
    ...
    if args.mode == "smoke":
        ...
        result = {
            "run_id": args.run_id,
            "experiment": "CEGSP-E1 cross-model APG memory smoke",
            "status": "complete",
            "config": vars(args),
    ...
    layer_candidates = {
        layer: build_top_candidates(codes, grads, layer, args.top_k_per_module)
        for layer in layers
    }
    layer_ranking = []
    for layer in layers:
        top = nonconflicting_prefix(layer_candidates[layer], max(parse_csv_ints(args.apg_ks)))
        layer_ranking.append({
            "layer": layer,
            "num_kept_candidates": len(layer_candidates[layer]),
            "num_nonconflicting_candidates": len(top),
            "top64_score_sum": float(sum(e.score for e in top[:64])),
            "top_score": float(top[0].score) if top else float("nan"),
        })
    layer_ranking.sort(key=lambda row: (-float(row["top64_score_sum"]), int(row["layer"])))

    apg_curve: List[Dict[str, object]] = []
    previous_val = affine_metrics["val"]
    selected_k = 0
    selected_row: Dict[str, object] | None = None
    stop_reason = "max_K_reached"
    for k in parse_csv_ints(args.apg_ks):
        edits: List[AffineEdit] = []
        for layer in layers:
            edits.extend(nonconflicting_prefix(layer_candidates[layer], k))
        edits.sort(key=lambda e: (-e.score, e.layer, e.key, e.row, e.block))
        ...
        row = {
            ...
            "score_sum": float(sum(e.score for e in edits)),
        ...
        apg_curve.append(row)
        partial = {
            "run_id": args.run_id,
            "status": "running",
            "completed_K": [int(x["K_per_layer"]) for x in apg_curve],
            "affine_baseline_metrics": with_ppl(affine_metrics),
            "apg_curve": apg_curve,
            "elapsed_sec": time.time() - started,
        }
        (out_dir / "e1_apg_partial.json").write_text(json.dumps(partial, indent=2, ensure_ascii=False))
        log(f"APG K={k}: val_nll={metrics['val']:.8f} edits={len(edits)}")
        if row["finite"] and metrics["val"] < previous_val:
            selected_k = k
            selected_row = row
            previous_val = metrics["val"]
        else:
    ...
    if selected_row is None:
        selected_row = {
            ...
            "score_sum": 0.0,
            "metrics": with_ppl(affine_metrics),
            "delta_vs_affine_nll": metric_delta(affine_metrics, affine_metrics),
            "audit": baseline_audit,
            "finite": all(math.isfinite(v) for v in affine_metrics.values()),
        }
    apply_affine_patch(model, codes)

    result = {
        "run_id": args.run_id,
        "experiment": "CEGSP-E1 cross-model complete TernRefine APG",
        "status": "complete",
        "config": vars(args),
        "data_source": data_source,
        "protocol": {
            ...
            "apg_schedule": parse_csv_ints(args.apg_ks),
            "apg_selection": "validation first non-improvement",
            "patch_unit": "K legal non-conflicting CPSR relocations per Q/K layer",
        ...
        "apg_curve": apg_curve,
        "selected_K_per_layer": selected_k,
        "selected": selected_row,
        "stop_reason": stop_reason,
        "elapsed_sec": time.time() - started,
    }
    out_path = out_dir / "e1_apg_result.json"
    out_path.write_text(json.dumps(result, indent=2, ensure_ascii=False))
    log(f"wrote {out_path}")
