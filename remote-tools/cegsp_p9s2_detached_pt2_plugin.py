...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sidecar-dir-name", default="pt2_llama7b_sidecar")
...
def tensor_int_sha256(tensor: torch.Tensor) -> str:
    payload = ",".join(str(int(x)) for x in tensor.reshape(-1).tolist()).encode("ascii")
...
def build_codes(
    ...
    for (_, (layer, name, module)) in enumerate(specs):
        ...
        parity_rows.append({
            ...
            "permutation_sha256": tensor_int_sha256(perm),
        })
    return codes, perms, {"rows": parity_rows}


def save_detached_artifacts(
    out_dir: Path,
    args: argparse.Namespace,
    codes: Dict[int, Dict[str, AffineCode]],
    perms: Dict[int, Dict[str, torch.Tensor]],
    fp_qk: Dict[int, Dict[str, torch.Tensor]],
    pt2_qk: Dict[int, Dict[str, torch.Tensor]],
    parity: Dict[str, object],
) -> Dict[str, object]:
    sidecar_dir = out_dir / args.sidecar_dir_name
    model_dir = sidecar_dir / "model"
    ...
    for layer, layer_codes in codes.items():
        for key, code in layer_codes.items():
            module_id = f"{layer}.{key}"
            ternary_payload["modules"][module_id] = {
                "layer": int(layer),
                "key": key,
                "T": code.T.cpu().contiguous(),
                "mu": code.mu.cpu().contiguous(),
                "alpha": code.alpha.cpu().contiguous(),
                "valid": code.valid.cpu().contiguous(),
                "original_shape": tuple(int(x) for x in code.original_shape),
                "group_size": int(code.group_size),
                "ssr_perm": perms[layer][key].cpu().contiguous(),
            }

    torch.save(ternary_payload, sidecar_dir / "ternary_state.pt")
    torch.save(fp_qk, sidecar_dir / "fp_qk.pt")
    ...
    metadata = {
        ...
        "config": vars(args),
        "artifacts": {
            "ternary_state": str(sidecar_dir / "ternary_state.pt"),
            "fp_qk": str(sidecar_dir / "fp_qk.pt"),
            "qk_checkpoint": str(model_dir / "qk_checkpoint.pt"),
        },
        "state_parity_at_export": {
            "pass": bool(parity["pass"]),
            "qk_module_count": int(parity["qk_module_count"]),
            "max_capture_codebook_residual": float(parity["max_capture_codebook_residual"]),
            "max_final_vs_capture_q_residual": float(parity["max_final_vs_capture_q_residual"]),
        },
    }
    (sidecar_dir / "metadata.json").write_text(json.dumps(metadata, indent=2, ensure_ascii=False))
    return metadata


def load_detached_artifacts(sidecar_dir: Path) -> Tuple[Dict[int, Dict[str, AffineCode]], Dict[int, Dict[str, torch.Tensor]], Dict[int, Dict[str, torch.Tensor]]]:
    ...
    for row in payload["modules"].values():
        layer = int(row["layer"])
        key = str(row["key"])
        perm = row["ssr_perm"].long()
        fp_original = fp_qk[layer][key]
        fp_perm = fp_original[:, perm]
        fp_padded, valid_from_fp = pad_permuted(fp_perm, int(row["group_size"]))
        valid = row["valid"].bool()
        if valid.shape != valid_from_fp.shape or bool((valid != valid_from_fp).any().item()):
            raise RuntimeError(f"valid mask mismatch after detached reload layer={layer} key={key}")
        code = AffineCode(
            mu=row["mu"].float(),
            alpha=row["alpha"].float(),
            T=row["T"].to(torch.int8),
            valid=valid,
            original_shape=tuple(int(x) for x in row["original_shape"]),
            group_size=int(row["group_size"]),
            fp_padded=fp_padded,
        )
        codes.setdefault(layer, {})[key] = code
        perms.setdefault(layer, {})[key] = perm
    return codes, perms, qk_checkpoint


def detached_reload_gate(
    model: torch.nn.Module,
    codes: Dict[int, Dict[str, AffineCode]],
    perms: Dict[int, Dict[str, torch.Tensor]],
    ...
    return {
        "pass": len(rows) == 64 and max_sidecar < 1e-3 and max_deployed < 1e-3 and all(bool(row["ssr_bijection"]) for row in rows),
...
def main() -> None:
    ...
    log(f"exported detached PT2 sidecar to {Path(sidecar_metadata['artifacts']['ternary_state']).parent}")

    sidecar_dir = Path(sidecar_metadata["artifacts"]["ternary_state"]).parent
    detached_codes, detached_perms, detached_qk = load_detached_artifacts(sidecar_dir)
    detached_reload = detached_reload_gate(model, detached_codes, detached_perms, detached_qk)
    codes = detached_codes
    perms = detached_perms
    pt2_qk = detached_qk
    ...
    if not parity["pass"] or not detached_reload["pass"]:
        result["performance_gate"] = "NOT_RUN_STATE_PARITY_FAILED"
        result["elapsed_sec"] = time.time() - started
        path = out_dir / "p9s2_result.json"
        path.write_text(json.dumps(result, indent=2, ensure_ascii=False))
        log(f"detached interface failed; wrote {path}")
        return
    ...
    result["performance_gate"] = {
        ...
        "detached_post_quantization_interface_pass": bool(detached_reload["pass"]),
        "classification": "PASS_DETACHED_POST_QUANTIZATION_INTERFACE" if detached_reload["pass"] else "DETACHED_INTERFACE_FAIL",
    }
    result["elapsed_sec"] = time.time() - started
    path = out_dir / "p9s2_result.json"
    path.write_text(json.dumps(result, indent=2, ensure_ascii=False))
