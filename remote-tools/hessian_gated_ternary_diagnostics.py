#!/usr/bin/env python3
"""Mechanism gate for validation-gated Hessian-aware ternary refinement.
...
def summarize(rows):
    ...
    variants = ("itf", "fixed_t_fit", "ungated_refine", "gated_refine")
    summary = {}
    ...
    gated = summary["gated_refine"]
    ungated = summary["ungated_refine"]
    gate = {
        "median_test_improvement_target": 0.02,
        "test_win_rate_target": 0.65,
        "gated_not_worse_than_ungated_target": True,
...
def main():
    ...
    for module_name in sorted(weights):
        ...
        for block_start in starts_for_width(
            ...
            ungated_q, _, ungated_info = refine_ternary(
                weight, ternary, allcal_s, None, args.max_steps
            )
            gated_q, _, gated_info = refine_ternary(
                weight, ternary, fit_s, validation_s, args.max_steps
            )
            variants["ungated_refine"] = ungated_q
            variants["gated_refine"] = gated_q
            ...
            row = {
                ...
                "ungated_changed_fraction": ungated_info["changed_fraction"],
                "ungated_acceptance_rate": ungated_info["acceptance_rate"],
