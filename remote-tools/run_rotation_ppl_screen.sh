...
script_path="${2:-/root/PT2-LLM-official/rotation_ppl_screen.py}"
layers="${3:-0,10,20,31}"
python_bin=/root/PT2-LLM/venv/bin/python
mkdir -p "$run_dir"
start_iso="$(date -Is)"
start_epoch="$(date +%s)"
printf 'status\tRUNNING\nstart\t%s\nlayers\t%s\n' "$start_iso" "$layers" > "$run_dir/status.tsv"
...
set +e
for variant in identity activation_rms_hadamard; do
  OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8 PT2_DATA_ROOT=/root/PT2-LLM/data \
    "$python_bin" "$script_path" \
      ...
      --variant "$variant" \
      --output-dir "$run_dir" \
      --layers "$layers" --nsamples 8 --tokens-per-sample 128 \
      --blocksize 128 --ppl-seqlen 2048 --eval-datasets wikitext2,c4 \
...
printf 'status\t%s\nstart\t%s\nend\t%s\nelapsed_seconds\t%s\nexit_code\t%s\npeak_gpu_mib\t%s\nlayers\t%s\n' \
  "$status" "$start_iso" "$end_iso" "$elapsed" "${run_rc:-1}" "$peak_mib" "$layers" > "$run_dir/status.tsv"
exit "${run_rc:-1}"
