#!/usr/bin/env python3
"""Clean-room TDBT2 support-transport experiment.

This script tests whether ternary support swaps have value before we build a
larger optimizer.  It never loads QAT checkpoints, QAT logits, QAT latent
weights, or QAT state priors.  The only backward path is an optional single
gradient of the composed-operator proxy at the deployed ternary point.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--operators", default="qk,vo")
    ...
    p.add_argument("--eval-candidates", type=int, default=24)
    p.add_argument("--max-swaps", type=int, default=8)
    p.add_argument("--tau", type=float, default=1.05)
    p.add_argument("--grad-batches", type=int, default=2)
...
def build_wikitext_splits(
    ...
    fit = make(train_text, fit_batches, 0)
    val = make(valid_text, val_batches, 0)
    untouched_offset = val_batches * batch_size * (seq_len + 1)
...
def main() -> None:
    ...
    nll_sanity = None
    if args.nll_sanity:
        fp_nll = evaluate_nll(model, val, device)
        apply_direct_ptq_local(model, args.group_size, args.threshold_factor)
        ptq_nll = evaluate_nll(model, val, device)
        nll_sanity = {"fp_val_nll": fp_nll, "ptq_val_nll": ptq_nll}
    del model
    ...
    for layer in layers:
        ...
        for operator in operators:
            ...
            f_candidates = forward_candidates(codes, xs_by_split["fit"], operator, args.candidate_pool)
            ...
            g_candidates = gradient_candidates(codes, grads, operator, args.candidate_pool)

            variants: List[Dict[str, object]] = []
            qg_states, qg_trace = run_one_shot(base_states, codes, g_candidates, args.max_swaps)
            variants.append(summarize_variant("QG-one-shot", qg_states, qg_trace, codes, xs_by_split, targets_by_split, operator, device))

            endpoint_f_states, endpoint_f_trace = run_greedy(
                base_states, codes, f_candidates, xs_by_split["fit"], targets_by_split["fit"],
                operator, device, args.max_swaps, args.eval_candidates, args.tau, use_barrier=False
            )
            variants.append(summarize_variant("endpoint-greedy-F", endpoint_f_states, endpoint_f_trace, codes, xs_by_split, targets_by_split, operator, device))

            tdbt_f_states, tdbt_f_trace = run_greedy(
                base_states, codes, f_candidates, xs_by_split["fit"], targets_by_split["fit"],
                operator, device, args.max_swaps, args.eval_candidates, args.tau, use_barrier=True
            )
            variants.append(summarize_variant("TDBT2-F", tdbt_f_states, tdbt_f_trace, codes, xs_by_split, targets_by_split, operator, device))

            endpoint_g_states, endpoint_g_trace = run_greedy(
                base_states, codes, g_candidates, xs_by_split["fit"], targets_by_split["fit"],
                operator, device, args.max_swaps, args.eval_candidates, args.tau, use_barrier=False
            )
            variants.append(summarize_variant("endpoint-greedy-G", endpoint_g_states, endpoint_g_trace, codes, xs_by_split, targets_by_split, operator, device))

            tdbt_g_states, tdbt_g_trace = run_greedy(
                base_states, codes, g_candidates, xs_by_split["fit"], targets_by_split["fit"],
                operator, device, args.max_swaps, args.eval_candidates, args.tau, use_barrier=True
            )
            variants.append(summarize_variant("TDBT2-G", tdbt_g_states, tdbt_g_trace, codes, xs_by_split, targets_by_split, operator, device))
            ...
            results.append(
                {
                    ...
                    "candidate_counts": {"forward": len(f_candidates), "gradient": len(g_candidates)},
                    "variants": variants,
                }
            )
    ...
    result = {
        ...
        "metric_definition": {
            ...
            "local_trust": "matrix-output NMSE to FP must not exceed tau times direct-PTQ local NMSE",
