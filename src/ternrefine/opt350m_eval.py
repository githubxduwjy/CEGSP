...
source.  It reports NLL, PPL, wall-clock, peak memory, and gap closure.
"""
...
from ternrefine.ce_gradient import (
    collect_ce_qk_grads,
    projection_weight,
    set_projection_weight,
    target_modules,
)
from ternrefine.data_eval import (
...
def run_qat_controls(
    ...
    untouched_c4: List[torch.Tensor],
    device: torch.device,
    fp_qk: Dict[int, Dict[str, torch.Tensor]],
    direct_qk: Dict[int, Dict[str, torch.Tensor]],
    layers: List[int],
    ...
    for steps in step_counts:
        for eta in etas:
            ...
            c4_nll = evaluate_nll(model, untouched_c4, device) if untouched_c4 else None
            rows.append(
                {
                    ...
                    "val_ppl": safe_ppl(float(val_nll)),
                    "untouched_w_nll": float(untouched_nll),
                    "untouched_w_ppl": safe_ppl(float(untouched_nll)),
                    "untouched_c4_nll": float(c4_nll) if c4_nll is not None else None,
                    "untouched_c4_ppl": safe_ppl(float(c4_nll)) if c4_nll is not None else None,
                    "dtype": dtype_label,
                }
            )
    restore_qk(model, layers, direct_qk)
...
def main() -> None:
    ...
    if untouched_c4:
        fp_nll["untouched_c4"] = evaluate_nll(model, untouched_c4, device)
    ...
    if untouched_c4:
        direct_nll["untouched_c4"] = evaluate_nll(model, untouched_c4, device)
    ...
    denom_c4 = (
        float(direct_nll["untouched_c4"] - best_multi["untouched_c4_nll"])
        if best_multi and untouched_c4 and best_multi.get("untouched_c4_nll") is not None
        else float("nan")
    )
    gap_ratio_c4 = (
    ...
    result = {
        ...
        "data": {
            ...
            "split": "Wikitext-2 train fit / validation val and later validation untouched; optional C4 validation is report-only transfer",
        },
        "environment": {
